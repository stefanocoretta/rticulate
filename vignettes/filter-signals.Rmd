...
it01_up <- it01_dlc |> 
  reframe(
    resample_signal(Y_sm, `Time of sample in annot`, by = 2), .by = c(knot_displ_id, knot)
  ) |> 
  mutate(
    signal_sm = filter_signal(signal, window_length = 11, apply = 2),
...
it01_up |> 
  ...
  geom_point(aes(y = signal_sm), colour = "red", size = 0.25) +
...
it01_up |> 
  filter(knot_displ_id == 14) |> 
  mutate(
    signal_sm_vel = get_velocity(signal_sm)
  ) |> 
  ggplot(aes(time, signal_sm_vel)) +
  geom_point() +
  geom_line(aes(y = abs(signal_sm_vel)), colour = "red") +
