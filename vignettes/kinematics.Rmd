...
For the `get_landmarks()` function to function correctly, the user has to select a time window which includes the displacement signal that roughly corresponds to the closing and opening gesture so that the calculated velocity contains the three minimum absolute velocities (which means the absolute velocity will have two peaks only). If the function detects more than two peaks, it does not return landmarks.

```{r get-landmarks, warning=FALSE}
# Filter signal to include only the necessary window
...
it01_up_lands <- it01_up |> 
  ...
  reframe(
    get_landmarks(Y_sm_up, time_int),
    .by = displ_id
  )
...
At the moment the algorithm is very crude and will reject any case in which there aren't precisely two peaks in the absolute velocity in the given time window.
...
maxdisp <- get_zerocross(it01_14$Y_sm_up_vel, it01_14$time_int, 0.4, 0.5)

get_peakvel(it01_14$Y_sm_up_vel, it01_14$time_int, maxdisp)
```
