...
For example, let's add a smooth to the model we used above and an interaction of this smooth with the one over `X`. For illustrative purposes, we will set up a smooth over `TR_abs_velocity`, which is the absolute velocity of the tongue root at the time point the tongue contour was extracted (note that this analysis might not make sense, and it is given here only to show how to extract the predictions).

```{r polar-place-2}
polar_place_2 <- polar_gam(
  Y ~
    s(X, by = c2_place) +
...
summary(polar_place_2)
```

We can now obtain the predicted tongue contours. We set specific values for `TR_abs_velocity` using the `values` argument.

```{r place-pred}
place_pred <- predict_polar_gam(polar_place_2, values = list(TR_abs_velocity = seq(2, 24, 5)))
...
If you want to add confidence intervals, the function `geom_polar_ci()` can help.

You first need to get the CI data from the model with `predict_polar_gam(return_ci = TRUE)`.

```{r ci-data}
ci_data <- predict_polar_gam(polar_place_2, return_ci = TRUE, values = list(TR_abs_velocity = seq(2, 24, 5)))
```

Now we can use the CI data with `geom_polar_ci()` (note the separate `aes()` in `geom_path()` and `geom_polar_ci()`).

```{r place-pred-ci, fig.width=7, fig.height=5}
place_pred %>%
  ggplot(aes(X, Y)) +
  geom_path(aes(colour = as.factor(TR_abs_velocity), linetype = as.factor(TR_abs_velocity))) +
  facet_grid(c2_place ~ .) +
  geom_polar_ci(data = ci_data, as.factor(TR_abs_velocity))
```
