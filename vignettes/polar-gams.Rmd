...
ci_data <- predict_polar_gam(
  ...
  filter(word == "paca") # filter by choosing any level of word
```

Now we can use the CI data with `geom_polar_ci()` (note the separate `aes()` in `geom_path()` and `geom_polar_ci()`). The confidence intervals are ridiculously wide in this example, because of the random smooth (which is probably not necessary in this model, but I added it to illustrate how to remove terms).
